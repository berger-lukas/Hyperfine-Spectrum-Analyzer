...
5. Set SPFIT/SPCAT paths on the new computer. A stale JSON override wins over
   environment/PATH settings. See the [installation walkthrough](INSTALL.md).
...
Do not overwrite code while the server is running. After transferring working
LIN files, verify quantum numbers, line counts and uncertainties. To roll back,
stop v7, restore the backups and start the original environment.
...
display switch restores original experimental intensities. English setup and
...
Use `7.0.0-rc.1` for user testing, then publish `7.0.0` once cross-platform CI
and native external-program smoke tests are complete. Use `7.0.1` for compatible
bug fixes and `7.1.0` for compatible new features. Consider a major version for
incompatible data or interface changes. Each update should explain what changed,
which existing configurations are affected, how to migrate, how to roll back,
and what was actually tested. Preserve version history instead of replacing it
with an unreviewed bulk upload.
